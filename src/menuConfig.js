/** Ön Büro (mystoneinn / role_id 14) — tüm mobil kullanıcılar sadece bunları görür. */
...
export function getAllowedMenuItems(admin) {
  if (!admin) return [];
  return MOBILE_MENU_ITEMS.filter((item) => MOBILE_FRONT_DESK_PERMS.includes(item.perm));
}
